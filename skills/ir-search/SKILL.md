...
**먼저 프로젝트 폴더에서 `ir-search-profile.md`를 찾는다.** 없으면 저장소 루트의 `ir-search-profile.template.md`를 안내한다 — 사용자가 이 양식을 채워 `ir-search-profile.md`로 저장(업로드)하면 그것을 프로필로 쓴다(질문 생략). 있으면 내용을 요약해 보여주고 "바뀐 것 있나요?" **한 번만** 확인한 뒤 바로 1단계로 간다 — 같은 질문을 조사 때마다 반복하는 것이 이 스킬의 가장 큰 마찰이므로, 프로필이 있으면 아래 질문은 생략한다.
...
### 5단계 — HTML 대시보드 생성 (보고서와 함께)

보고서 md를 쓴 뒤 **같은 폴더에 `dashboard.html` 한 파일**을 만든다. 사용자가 "대시보드", "한눈에", "화면으로 보여줘"라고 하지 않아도 기본으로 만든다 (파일 하나라 부담이 없다). 형식은 `references/dashboard_schema.md`, 템플릿은 `templates/dashboard.html`(위니브 표준 서식 기반, 파랑 계열).
...
python3 "${CLAUDE_PLUGIN_ROOT}/skills/ir-search/scripts/build_dashboard.py" build dashboard.json -o dashboard.html
```

**② 판정 기록 규칙** — 4단계 3분류 결과를 그대로 옮긴다. 보고서와 대시보드의 판정이 달라서는 안 된다:

- **A그룹 → `open[]`에 남기고** `score`(0~100)·`fit`·`sum`·`eligibility`·`warn`을 채운다. `fit`은 `<b class='kw'>(키워드)</b> 설명` 형식으로 2~3개 (샘플: `(과업 일치)`, `(자격)`, `(경쟁도)`). 지역 제한 공고는 `region`에 지역 이름
- **B그룹 → `roadmap[]`로 이동** (`when`=트리거·다음 회차, `point`=연쇄 경로). `open[]`에서는 제거
- **C그룹 → `reframe[]`로 이동** (`angle`=재서술 각도, `risk`)
- **탈락·부재 확인 → `excluded[]`** (`reason`에 미충족 요건). 유명 사업의 부재도 여기 적는다
- **상세 검증을 안 한 A후보**는 `open[]`에 두되 `score=null`로 남긴다 → 화면에 "미평가 / 검증 전"으로 표시된다. **점수를 지어내지 않는다**
- `conclusion`: 보고서의 "우선순위 액션"을 `<b class="h">` 소제목 + `<ul>`로. `notes`: 커버리지(N건/전체, partial 소스, api-window), 불명 표기 안내
- 수주(입찰) 공고는 `kind="bid"`, `budget/bl/method`에 추정가격·계약방식을 넣는다 (나라장터 크롤러 추가 시)

**재조사(diff 모드)**에서는 새 폴더에서 `bootstrap --merge -o <직전 dashboard.json 복사본>`으로 돌리면 직전 판정(UNCHANGED)을 보존하고 신규 건만 미평가로 들어온다. NEW/CHANGED 건의 판정을 채운 뒤 build 한다.

채팅 응답에는 `dashboard.html` 경로를 보고서 경로와 함께 알린다. 브라우저에서 열면 상단 토글(전체/수주/지원사업)·검색·유형 칩·정렬·지역 한정 버튼으로 걸러 볼 수 있다.
